...
description: >-
  検証 skill の feature map がアプリの実態からずれていないかを定期的に棚卸しする。
  機能ごとに read-only subagent をソース調査に並列で当て、そのあとコーディネータが全機能を実際に 1 回ずつ動かす。
  結果は clean / changed / blocked のいずれかを必ず宣言し、証明済みの修正だけを PR 1 本にまとめる。
  実行中にプロダクトコードは編集しない。
  Use for /maintain-verification-skill, "verify skill を監査して", or auditing a project's feature map for drift.

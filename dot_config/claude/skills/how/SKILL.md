...
name: how
description: >-
  「これはどう動いているのか」に、シニアがオンボードする粒度で答える。
  複雑なサブシステムは探索 subagent を並列に走らせてから 1 つの説明に統合する。
...
- `subagent_type`: `Explore` (read-only search agent; it reads excerpts rather than whole files, which is exactly the explorer's job)
- `model`: omit to inherit, or `sonnet` for a wide sweep
...
- `subagent_type`: `general-purpose`
- `model`: `opus` for one, `fable` for the other. Escalate or add a third only when the architecture warrants it.

Weight a finding by the argument it makes, not by how many critics raised it.

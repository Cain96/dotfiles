...
description: >-
  長時間・無人・多段階の作業で、後から人がレビューできる意思決定の軌跡を TSV で残す。
  1 行 1 決定（何を・なぜ・証拠・結果）。既定はローカル、レビュアーが軌跡を見ないと結果を信用できない規模のときだけコミットする。
  最後にログが嘘をついていないかを実際の記録と突き合わせて監査する。
  「寝てる間に進めて」「戻ったとき信用できる形で」と言われたときに使う。
  Use for /show-me-your-work, autonomous or multi-phase runs, or work a human reviews after stepping away.
...
At the end of the run, before handing back, check the log told the truth. This session's record lives under `~/.config/claude/projects/<slug>/<session-id>/`, where `<slug>` is the workspace path with each `/` turned into `-` (so `/Users/you/proj` becomes `-Users-you-proj`). Read only this session's directory. Never glob across `~/.config/claude/projects/*/`; that reads unrelated private chats from other projects. If you cannot resolve this session's directory, audit the log against the conversation you can see and say that the transcript check was not available, rather than skipping the audit silently.
...
## Flag what still deserves scrutiny
...
- Verification steps skipped, or claimed without proof anywhere in the run.
- Choices that look risky in hindsight (premature, scope-creeping, papering over a symptom).
- Gaps the user would otherwise miss on a casual skim.

Every reply for a run that produced a trail ends with an "Attention" section listing each flag against specific rows or moments. "No flags" is a valid value.

...
name: blast-radius
description: >-
  小さく見える変更が「diff の外」で何を壊すかを洗い出し、安全である理由となっている 1 つの事実を
  文章ではなく実際にコードを走らせて証明する。確信度は 5 段階で、どこで止まったかを必ず宣言する。
  「これ何を壊す？」「影響範囲は」「この差分が不安」と言われたとき、
  共通処理・型・API レスポンス・DB カラム・feature flag に触れる変更をレビューするときに使う。
  Use for "blast radius of X", "what could this break", 影響範囲調査, or reviewing a small diff you don't trust.
...
6. For a big or wide change, ask the same question to two subagents on different models (`opus` and `fable`, `subagent_type: general-purpose`) and merge the answers. Both are Anthropic-family, so treat agreement as ordinary corroboration rather than independent confirmation; a finding only one of them raises is still worth chasing.

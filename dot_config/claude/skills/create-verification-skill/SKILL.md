...
description: >-
  そのリポジトリ専用の検証 skill を生成する。実アプリを起動し、ユーザーと同じ経路で機能を動かし、証拠を残す手順を
  `.claude/skills/verify-<app>/` に書き出す。feature map（機能ごとの入口・駆動方法・成功の観測条件）も併せて作る。
  読者は人間ではなく、このアプリを一度も見ていないエージェントである前提で書く。
  プロジェクトに「アプリの挙動をスクリプトで証明する手段」が無いときに使う。
  Use for /create-verification-skill, "この repo の検証 skill を作って", or when a project has no scripted way to prove UI/CLI/service behavior.

...
name: why
description: >-
  「なぜこの実装になっているのか」を、証拠を並列に集めて出典付きで答える。
  ソース管理・チケット・長文ドキュメント・チャット・インフラ可観測性・エラートラッキングを
  それぞれ専任の subagent が同時に調べ、空振りも一級の証拠として報告する。
...
Check which of these servers are actually connected this session before spawning. The mapping is fixed; only availability varies.

| # | Evidence category | Server in this environment |
|---|---|---|
| 1 | Source control history | `git` and `gh` (always available) |
| 2 | Issue / ticket tracker | Linear, Atlassian (Jira), GitHub Issues via `gh` |
| 3 | Long-form documents | Notion |
| 4 | Real-time team chat | Slack |
| 5 | Infrastructure observability | Datadog |
| 6 | Error / exception tracking | Sentry |
| 7 | Product analytics warehouse | **none. Permanent gap in this environment.** |

Category 7 has no server here and is not expected to gain one. Report it as a standing gap in Sources Consulted, not as a per-run "skipped" decision, and never let its absence become a reason to narrow the other six.

If a server in the table is not connected this session, that is a per-run gap. Say so in the coverage map. If a connected server does not appear in the table, classify it by the evidence it holds and record the ambiguity.
...
- `subagent_type`: `general-purpose`. **Do not use `Explore`.** It cannot reach MCP servers, which disables every MCP-backed investigator.
- `model`: omit to inherit the parent model. Investigators are search-bound, not judgment-bound.
- Investigators must not write anything. State that in the prompt. It is a posture, not a sandbox.
...
- `subagent_type`: `general-purpose`. The synthesizer spot-verifies citations, which can require MCP access, so `Explore` is not an option.
...
- Product analytics warehouse: no MCP in this environment. Standing gap. Usage trajectories, experiment exposure, and the distributions behind threshold constants were not searchable.

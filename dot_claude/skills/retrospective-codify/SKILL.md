...
| 手順・文脈判断・テンプレが必要 | 新規 skill または既存 skill への追記 | "monorepo に新パッケージを追加する手順" |
...
- 最初の試行: pnpm monorepo に新しいパッケージを追加したら、他パッケージから import できなかった / TypeScript の型解決が通らなかった。
- 最終解: `pnpm-workspace.yaml` への追加 + 依存側 `package.json` の `workspace:*` 参照 + ルート `tsconfig.json` の `references` 配線 + 追加後の `pnpm install` を一括で行う。
- 気付き: 単一手順では収まらず、workspace 設定・依存宣言・tsconfig references の 3 層を一括して理解する必要がある。

→ 新規 skill `pnpm-monorepo-add-package` として手順とテンプレを切り出す（重複チェックで該当 skill が既にあれば追記候補になる）。
